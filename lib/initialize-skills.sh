...
SKILLS_DIR="${HOME}/.config/ring/skills"
SKILLS_REPO="https://github.com/lerianstudio/ring.git"
...
if [ -d "${HOME}/.config/ring/.git" ]; then
    echo "Found existing installation. Backing up..."
    mv "${HOME}/.config/ring/.git" "${HOME}/.config/ring/.git.bak"

    if [ -d "${HOME}/.config/ring/skills" ]; then
        mv "${HOME}/.config/ring/skills" "${HOME}/.config/ring/skills.bak"
        echo "Your old skills are in ~/.config/ring/skills.bak"
    fi
fi

# Clone the skills repository
mkdir -p "${HOME}/.config/ring"
...
if command -v gh &> /dev/null; then
    echo ""
    echo "GitHub CLI detected. Would you like to fork ring-skills?"
    echo "Forking allows you to share skill improvements with the community."
    echo ""
    read -p "Fork ring-skills? (y/N): " -n 1 -r
    echo

    if [[ $REPLY =~ ^[Yy]$ ]]; then
        gh repo fork lerianstudio/ring-skills --remote=true
