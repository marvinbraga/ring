...
function analyzeCallGraph(filePaths: string[], targetFunctions?: string[]): CallGraphOutput {
  ...
  for (const filePath of existingFiles) {
    ...
    const functions = extractFunctions(sourceFile, filePath, typeChecker);
...
function extractCallInfo(
  ...
  function shouldIgnoreCall(): boolean {
    ...
    if (ts.isElementAccessExpression(node.expression)) {
      ...
      if (ts.isIdentifier(objExpr) && arg && ts.isStringLiteralLike(arg) && isIgnoredPropertyCall(objExpr.text, arg.text)) {
        return true;
      }
    }
  ...
  const isMethod = ts.isPropertyAccessExpression(node.expression) || ts.isElementAccessExpression(node.expression);
  ...
  function resolveCalledSymbol(): ts.Symbol | undefined {
    ...
    if (ts.isPropertyAccessExpression(node.expression)) {
      symbolFromLocation =
        typeChecker.getSymbolAtLocation(node.expression.name) ?? typeChecker.getSymbolAtLocation(node.expression);
    } else if (ts.isElementAccessExpression(node.expression)) {
      const arg = node.expression.argumentExpression;
      if (arg && ts.isStringLiteralLike(arg)) {
        symbolFromLocation = typeChecker.getSymbolAtLocation(arg) ?? typeChecker.getSymbolAtLocation(node.expression);
      } else {
  ...
  function getCanonicalTargetFromSymbol(sym: ts.Symbol): string | null {
    ...
    const decls = symbol.getDeclarations() ?? (symbol as unknown as { declarations?: ts.Declaration[] }).declarations;
...
function parseArgs(args: string[]): { files: string[]; functions: string[] } {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--functions' && i + 1 < args.length) {
      ...
      functions.push(...funcList.split(',').map((f) => f.trim()).filter((f) => f));
      i++;
