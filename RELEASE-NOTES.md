...
- Added unified `ring-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `ring:skill-name` for ring skills, `skill-name` for personal
- Personal skills override ring skills when names match
...
- Updated terminology: "ring skills" instead of "Core skills"

### Files Added
- `codex/INSTALL.md` - Installation guide for Codex users
- `codex/ring-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `scripts/ring-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core ring functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-ring skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-ring/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-ring skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-ring/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `ring:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `ring:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `ring:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `ring:code-reviewer`
...
- All internal skill references now use `ring:` namespace prefix
- Updated format: `ring:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/ring:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# ring v2.0.0 Release Notes

## Overview

ring v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([obra/ring-skills](https://github.com/obra/ring-skills)). This transforms ring from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [obra/ring-skills](https://github.com/obra/ring-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/ring/skills/`
...
1. Your old `~/.config/ring/.git` will be backed up to `~/.config/ring/.git.bak`
2. Old skills will be backed up to `~/.config/ring/skills.bak`
3. Fresh clone of obra/ring-skills will be created at `~/.config/ring/skills/`

### Removed Features

- **Personal ring overlay system** - Replaced with git branch workflow
- **setup-personal-ring hook** - Replaced by initialize-skills.sh
...
- Clones obra/ring-skills on first run
...
- `ring_SKILLS_ROOT` set to `~/.config/ring/skills`
...
- Removed obsolete setup-personal-ring call from session-start
...
- Prominent link to ring-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in obra/ring-skills
- `scripts/` directory - Now in obra/ring-skills/skills/using-skills/
- `hooks/setup-personal-ring.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/ring/skills
- `commands/brainstorm.md` - Updated paths to ring_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to ring_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to ring_SKILLS_ROOT
...
- PR #2: Personal ring overlay system (later replaced)
...
```bash
# In Claude Code
/plugin marketplace add obra/ring-marketplace
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/ring/skills ~/ring-skills-backup
   ```
...
- Skills repository is now at https://github.com/obra/ring-skills
...
**Full Changelog:** https://github.com/obra/ring/compare/dd013f6...main
**Skills Repository:** https://github.com/obra/ring-skills
**Issues:** https://github.com/obra/ring/issues
