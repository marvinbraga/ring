...
When `--prompt` is provided **without a tasks file**, dev-cycle enters prompt-only mode:
...
/dev-cycle --prompt "Implement multi-tenant support for the API"
...
- Refactoring work (use `/dev-refactor` first)
...
# Prompt-only mode (no tasks file needed)
/dev-cycle --prompt "Implement multi-tenant support with organization_id in all entities"
```

## Prerequisites

1. **Task file OR prompt**: Either a markdown file generated by `/pre-dev-*` or `/dev-refactor`, OR `--prompt` for prompt-only mode
