...
metadata:
  trigger: "Tests use setTimeout/sleep, flaky tests, timeouts in parallel runs"
  skip_when: "Testing actual timing behavior (debounce/throttle), synchronous tests"
  source: ring-default
---
...
**Generic polling:** `waitFor(condition, description, timeoutMs=5000)` - poll every 10ms, throw on timeout with clear message.
